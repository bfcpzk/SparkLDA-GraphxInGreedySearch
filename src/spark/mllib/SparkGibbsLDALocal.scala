...
object SparkGibbsLDALocal {
  ...
  /**
    ...
    */
  def saveDocTopicDist(documents: RDD[(String, String, Array[(Int, Int)], Array[Int])], pathTopicDistOnDoc: String): Any = {
    documents.map {
      case (userId, docId, topicAssign, nmk) =>
        ...
        val res = userId + ", " + docId + ", " + probabilities
        res
  ...
  def lda(filename: String, kTopic: Int, alpha: Double, beta: Double,
    ...
    val vSize = allWords_temp.length
    var wordIndexMap = new HashMap[String, Int]()
    for (i <- 0 until allWords_temp.length) {
      wordIndexMap(allWords_temp(i)._1) = allWords_temp(i)._2.toInt
    }
    val bWordIndexMap = wordIndexMap

    val allWords = allWords_temp.sortBy(x => x).map( l => l._1 ).toList*/
    ...
    var documents = rawFiles.map { t => //t means (docId,words) where words is a List
      val docId = t._2.toString
      val length = t._3.length
      val topicAssignArr = new Array[(Int, Int)](length)
      val nmk = new Array[Int](kTopic)
      for (i <- 0 until length) {
        val topic = Random.nextInt(kTopic)//随机生成一个主题编号
        topicAssignArr(i) = (bWordIndexMap(t._3(i)), topic) //((word1,topic1),(word2,topic2),...)长度为doc中的所有词
        nmk(topic) = nmk(topic) + 1 //doc中包含topic的次数
      }
      (t._1.toString, docId, topicAssignArr, nmk) //t._1 means userId, t._2 means docId, t._3 means words
    }.cache

    //资源回收
    rawFiles.unpersist(blocking = false)
    //当前topic k 下的word t 数量
    var wordsTopicReduced = documents.flatMap(t => t._3).map(t => (t, 1)).reduceByKey(_ + _).collect().toList
    //update nkv,nk
    ...
    for (iter <- 1 until maxIter + 1) {
      ...
      updatedDocuments = iterativeInputDocuments.map {
        case (userId, docId, topicAssignArr, nmk) =>
          //gibbs sampling
          val (newTopicAssignArr, newNmk) = gibbsSampling(topicAssignArr,
            nmk, nkv, nk,
            kTopic, alpha, vSize, beta)
          (userId, docId, newTopicAssignArr, newNmk)
      }

      //nkGlobal.unpersist(blocking = false)
      //nkvGlobal.unpersist(blocking = false)
      wordsTopicReduced = updatedDocuments.flatMap(t => t._3).map(t => (t, 1)).reduceByKey(_ + _).collect().toList
